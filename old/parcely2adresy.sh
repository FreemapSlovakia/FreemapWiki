...
if [[ $1 ]] && [[ $2 ]] && [[ $3 ]]
 ...
 if [[ ${name} ]]
 then
     echo "<node id='-"$nLine"' visible='true' lat='"$latitude"' lon='"$longitude"'><tag
 k='addr:city' v='"${meno_obce}"'/><tag k='addr:conscriptionnumber' v='"$supisne"' /><tag
 k='addr:country' v='SK' /><tag k='addr:housenumber' v='"$supisne"/"$orientacne"' /><tag
 k='addr:postcode' v='"${psc}"' /><tag k='addr:street' v='"$ulica"' /><tag k='addr:streetnumber'
 v='"$orientacne"' /></node>" >> ${out}
     echo "<node id='-"$nLine"' visible='true' lat='"$latitude"' lon='"$longitude"'><tag k='name'
 v='"$name"'/></node>" >> names.osm
 else
     echo "<node id='-"$nLine"' visible='true' lat='"$latitude"' lon='"$longitude"'><tag
 k='addr:city' v='"${meno_obce}"'/><tag k='addr:conscriptionnumber' v='"$supisne"' /><tag
 k='addr:country' v='SK' /><tag k='addr:housenumber' v='"$supisne"/"$orientacne"' /><tag
